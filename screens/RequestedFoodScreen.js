...
var C5 = '#E7E5DF'; //text
//list of food that the other users have requested
export default class RequestedFoodScreen extends Component{
  constructor(){
    super()
    this.state = {
  ...
  getrecievedFoodList =()=>{
    this.requestRef = db.collection("requested_Food").where('user_id','!=',this.state.userId).where("status", '==','requested').onSnapshot((snapshot)=>{
      ...
      <ListItem
        ...
        rightElement={
          <TouchableOpacity style={styles.button}
            onPress ={()=>{
              this.props.navigation.navigate("RequestedFoodDetails",{"details": item})
            }}
            >
  ...
  render(){
    return(
      <View style={{flex:1, backgroundColor:C1}}>
        <MyHeader title="Requested Pet Food" navigation ={this.props.navigation}/>
        <View style={{flex:1}}>
          {
